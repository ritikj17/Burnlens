...
import { ArrowRight, Calendar, CheckCircle2, CircleDollarSign, ShieldCheck } from "lucide-react";
...
function ConfidenceBadge({ confidence }: { confidence: string }) {
  const variant = confidence === "high" ? "success" : confidence === "medium" ? "warning" : "muted";
  return <Badge variant={variant}>{confidence} confidence</Badge>;
}

export function ReportView({ report, shareUrl }: { report: PublicAuditReport; shareUrl: string }) {
  const result = report.result;
  const recommendCredex = getCredexConsultationRecommended(result);
        ...
        <div className="container py-12 md:py-16">
          <div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="no-print flex flex-col gap-3 sm:flex-row">
              <ShareButton url={shareUrl} title="BurnLens AI spend audit" />
              <PrintButton />
            </div>
          </div>
        </div>
          ...
          {recommendCredex ? (
            <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-6">
              <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                ...
                <Button asChild>
                  <a href={getCredexConsultationUrl()} target="_blank" rel="noreferrer">
                    Book Credex consult
          ...
          <div>
            ...
            <div className="space-y-4">
              {result.recommendations.map((recommendation) => (
                <Card key={`${recommendation.toolId}-${recommendation.category}`}>
                  <CardContent className="p-5">
                    <div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
                      <div>
                        <div className="flex flex-wrap items-center gap-2">
                          <h3 className="text-lg font-semibold tracking-normal">{recommendation.toolName}</h3>
                          <ConfidenceBadge confidence={recommendation.confidence} />
                        </div>
                        <p className="mt-2 text-sm text-muted-foreground">
                          {recommendation.currentPlan} · {recommendation.currentSeats} seat{recommendation.currentSeats === 1 ? "" : "s"} · {formatCurrency(recommendation.currentMonthlySpend)}/mo
                        </p>
                        <p className="mt-4 font-medium">{recommendation.recommendedAction}</p>
                      ...
                      <div className="shrink-0 rounded-lg border bg-muted/45 p-4 text-left md:w-48">
                        ...
                        {recommendation.recommendedPlan ? (
                          <span className="mt-3 block text-xs text-muted-foreground">
                            Target: {recommendation.recommendedPlan}
                          </span>
                        ) : null}
                      </div>
                    </div>
          ...
          <Card>
            <CardContent className="grid gap-4 p-5">
              <div className="flex items-center justify-between">
                ...
                <strong>{formatCurrency(result.totalCurrentMonthlySpend)}/mo</strong>
              </div>
              <div className="flex items-center justify-between">
              ...
              <div className="flex items-center justify-between">
                ...
                <strong>{new Date(report.createdAt).toLocaleDateString("en-US")}</strong>
              </div>
            </CardContent>
          </Card>
